...
enum EntityType    { PLAYER, WATER, GRASS, PLANE, NONE };
enum ChickenStatus { WIN, LOSE, INGAME };
...
public:
    ...
    static constexpr int   DEFAULT_SPEED         = 500;
    static constexpr float DEFAULT_FRAME_SPEED   = 14.0f;
    static constexpr float Y_COLLISION_THRESHOLD = 0.5f;
    static constexpr int   DEFAULT_ENERGY        = 1000;
    ...
    void update(float deltaTime, Entity *water, int waterCount, 
                Entity *grass, int grassCount, Entity *plane, int planeCount);
    void render();
    ...
    void rotateClockwise() { if(mEntityStatus == ACTIVE) mAngle += 1.0f; }
    void rotateCounterClockwise() { if(mEntityStatus == ACTIVE) mAngle += -1.0f; }

    void accelerate() { mAcceleration.x = sin(mAngle * PI / 180.0f) * 60.0f; 
                        mAcceleration.y = -cos(mAngle * PI / 180.0f) * 60.0f; 
                        if (mEnergy >= 0 && mEntityStatus != INACTIVE) mEnergy -= 1; }

    void decelerate() { mAcceleration.x = (mVelocity.x >= 0) ? -5.0f : 0.0f; 
                        mAcceleration.y = 20.0f; }
    ...
    ChickenStatus getChickenStatus() const { return mChickenStatus; }
