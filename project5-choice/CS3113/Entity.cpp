...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mEntityType {NONE}, mLives {2}, mShaderStatus {0} { }
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
    ...
    mAngle {0.0f}, mEntityType {entityType}, mLives {2}, mShaderStatus {0} { }
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        ...
        mSpeed { DEFAULT_SPEED }, mEntityType {entityType}, mLives{2}, 
        mShaderStatus {0} { }
    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (collidableEntity->mEntityType == NPC) {
                if (mDamageTimer <= 0) {
                    collidableEntity->setAIState(ENGAGE);
                    mDamageTimer = 2.0f;
                }
            } else if (collidableEntity->mEntityType == CARROT) {
                collidableEntity->deactivate();
                mShaderStatus = 0;
            } else if (collidableEntity->mEntityType == POTION) {
                collidableEntity->deactivate();
                mSpeed = 300;
            }
        }
...
void Entity::AIWander() {
    if (mPosition.y <= -750.0f) moveDown();
    else if (mPosition.y >= -150.0f) moveUp();
}
...
void Entity::AITeleport() {
    if (mAIState != ENGAGE) {
        if (mPosition.x == 60.0f && mTeleportTimer <= 0) {
            mPosition.x = 1550.0f;
            mPosition.y = 150.0f;
            mTeleportTimer = 5.0f;
        } else if (mPosition.x == 1550.0f && mTeleportTimer <= 0) {
            mPosition.x = 60.0f;
            mPosition.y = 750.0f;
            mTeleportTimer = 5.0f;
        }
...
{
    if (mEntityStatus == INACTIVE || mAIState == ENGAGE) return;
    ...
    if (mDamageTimer >= 0) mDamageTimer -= deltaTime;
    if (mTeleportTimer >= 0) mTeleportTimer -= deltaTime;
